...
updates_dir=/data/infinity_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have infinity-VERSION-DATE-TYPE-* as format"
...
# Assume infinity-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.infinity.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.infinity.updater/databases/updates.db" \
